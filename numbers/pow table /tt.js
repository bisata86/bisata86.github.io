	...
	myinterval =  setInterval(function(){
		if(operation=='add') {
			...
			if(Math.pow(juo, curCounter)=='Infinity') {
				...
				//console.log('o:'+juo)
				var sallame = juo;
				for (var culo = 1; culo < curCounter; culo++) {
					sallame = sallame * juo;
					//console.log(sallame)
				};
				//console.log(sallame)
				//if((Math.pow(juo, curCounter))%punti!=0) console.log('cane');
				console.log(juo+' alla '+curCounter+' = '+ sallame+', mod('+punti+') = '+(sallame)%punti)
				for (var l = newjuos.length - 1; l >= 0; l--) {
				if(newjuos.length!=0)
				if(newjuos[l].a==juo && newjuos[l].b==sallame%punti) {
				...
				newjuos.push({a:juo,b:sallame%punti})
		...
		if(juo>=elements.length && operation!='pow') {
			clearInterval(myinterval);
