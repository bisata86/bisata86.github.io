...
function nonmodificareIvaloriRealtime() {
...
addEventListener("keydown", function (e) {
	...
	if(e.keyCode==37)  { objects[objects.length-1].moveleft = true; }
	if(e.keyCode==39)   { objects[objects.length-1].moveright = true; }
}, false);
addEventListener("keyup", function (e) {
	...
	if(e.keyCode==37)  { objects[objects.length-1].moveleft = false; }
	if(e.keyCode==39)   { objects[objects.length-1].moveright = false; }
}, false);
$( document ).ready(function() {
	...
	$( "body" ).on( 'mousedown touchstart', '#rotater', function(e) {
	 ...
	 $( "body" ).on( 'mousedown touchstart', '#dx', function(e) {
		if(enablekeyboard)  { objects[objects.length-1].moveright = true; }
	 });
	 $( "body" ).on( 'mouseup touchend', '#dx', function(e) {
		if(enablekeyboard) { objects[objects.length-1].moveright = false; }
	 });
	$( "body" ).on( 'mousedown touchstart', '#sx', function(e) {
		if(enablekeyboard) { objects[objects.length-1].moveleft = true; }
	 });
	$( "body" ).on( 'mouseup touchend', '#sx', function(e) {
		if(enablekeyboard) { objects[objects.length-1].moveleft = false; }
	 });
...
var render = function() {
	...
	for (var ikl = objects.length - 1; ikl >= 0; ikl--) {
				...
				if(drawPoints) {//if(drawPoints && objects[ikl].fine==false) {
				    $.each(objects[ikl], function(key,val) {
...
var update = function () {
	...
	if(!gameover) {
		for (var i = objects.length - 1; i >= 0; i--) {
			...
			if(!objects[i].fine) {
				...
				var colpito2 = false;
				$.each(objects[i], function(key,val) {
						if(objects[i].moveleft  && enablekeyboard)  {
						...
						else if(objects[i].moveright && enablekeyboard)  {
							if(key.indexOf('cent')!=-1) {
								val[0] = val[0]+2;
							}
							if(key.indexOf('rim')!=-1){
						...
						if(((objects[i].rotateleft && enablekeyboard)) && !objects[i].eraser)  {
							if(key.indexOf('cent')!=-1) {
						...
						else if(((objects[i].rotateright && enablekeyboard)) && !objects[i].eraser)  {
							if(key.indexOf('cent')!=-1) {
									var a = val[0]; var b = val[1];
									val[0] = objects[i].center[0]+(Math.cos(2/180*Math.PI)*(a-objects[i].center[0])-Math.sin(2/180*Math.PI)*(b-objects[i].center[1]));
									val[1] = objects[i].center[1]+(Math.sin(2/180*Math.PI)*(a-objects[i].center[0])+Math.cos(2/180*Math.PI)*(b-objects[i].center[1]));
						...
						if(key.indexOf('rim2')!=-1) {
							for (var imk = val.length - 1; imk >= 0; imk--) {
								for (var dunno = thefinalperim.length - 1; dunno >= 0; dunno--) {
										if(
											val[imk][3]!=undefined &&
											val[imk][0]<thefinalperim[dunno][0]+hitDistance+distancefordisabling &&
											val[imk][0]>thefinalperim[dunno][0]-hitDistance-distancefordisabling &&
											val[imk][1]<thefinalperim[dunno][1]+hitDistance+distancefordisabling &&
											val[imk][1]>thefinalperim[dunno][1]-hitDistance-distancefordisabling
											)
										{
											//disableEvents =true;
							...
							for (var imk = val.length - 1; imk >= 0; imk--) {
								for (var dunno = thefinalperim.length - 1; dunno >= 0; dunno--) {
										if(
											val[imk][3]!=undefined &&
											val[imk][0]<thefinalperim[dunno][0]+hitDistance &&
											val[imk][0]>thefinalperim[dunno][0]-hitDistance &&
											val[imk][1]<thefinalperim[dunno][1]+hitDistance &&
											val[imk][1]>thefinalperim[dunno][1]-hitDistance
											)
										{
											colpito = true;
				...
				if(colpito) {
					...
					objects[i].lisiocounter = objects[i].lisiocounter+1;
				}
				if(objects[i].rotateleft) {
					objects[i].angle = (objects[i].angle+(2))%360;
				} else if (objects[i].rotateright) {
					objects[i].angle = (objects[i].angle+(358))%360;
				}
			}
			if(!objects[i].fine) {
				//console.log(objects[i].lisiocounter)
				//handle lisiocounter
				//if(objects[i].lisiocounter==5) {
					objects[i].revgrcenter = [objects[i].center[0],objects[i].center[1]]
				//}
				...
				if(!objects[i].moving && thepoint[1]>objects[i].center[1]) {
					...
					if(objects[i].inerzia==0) {
						...
						bounce(1)
						function bounce(angle) {
							var bounceangle = angle;
							objects[i].inerzia--;
							var center = objects[i].grcenter;
							//bounce upright
				...
				//move down
					...
					$.each(objects[i], function(key,val) {
						...
						if(key.indexOf('rim2')!=-1){
							for (var ik = objects[i].rim2.length - 1; ik >= 0; ik--) {
								objects[i].rim2[ik][1] = objects[i].rim2[ik][1]+disteee;
							};
						}
					});
	...
	function handleCollision() {
		...
		if(objects[i].hits.length>20
			...
			//	&& distanceBetween2(objects[i].hits[objects[i].hits.length-2],objects[i].hits[objects[i].hits.length-4]) < 10
				&& distanceBetween2(objects[i].grcenter,objects[i].revgrcenter)>15
		 	...
		 	if(debugcollisions)console.log('atterraggio su due punti');
			end();
		}
		 else   {
			if(thepoint[1]>objects[i].center[1] || true) {
				//console.log('colpo sotto');
				if((theperimpoint[2]>45 && theperimpoint[2]<135) || (theperimpoint[2]>225 && theperimpoint[2]<315) || true) {
				 	if(debugcollisions)console.log('parete verticale')
					if((theperimpoint[2]>225 && theperimpoint[2]<315)) { //if(objects[i].center[0]>thepoint[0]) {
						$.each(objects[i], function(key,val) {
							if(key.indexOf('cent')!=-1) {
								val[0] = val[0]+1;
								val[1] = val[1]-1;
					...
					else if((theperimpoint[2]>45 && theperimpoint[2]<135)) {
						$.each(objects[i], function(key,val) {
							if(key.indexOf('cent')!=-1) {
								val[0] = val[0]-1;
								val[1] = val[1]-1;
	...
	function end() {
		if(!allmoving) {
			...
			objects[i].rotateleft = false;
			objects[i].rotateright=false;
			objects[i].moveright = false;
			objects[i].moveleft = false;
			fastdown = false;
			if(!objects[i].eraser) {
				var theMinX =  10000;
...
function changevel() {
	clearInterval(myInterval);
	myInterval = setInterval(function(){ update(); }, 0);
}
}
nonmodificareIvaloriRealtime();
